...
interface MatrizCargaNoLectivaProps {
  ...
  alIntentarCelda?: (celda: Celda) => void;
  alArrastrarSobreCelda?: (celda: Celda) => void;
  seccionArrastrando?: string | null;
  bloqueado?: boolean;
...
export function MatrizCargaNoLectiva({
  ...
  alHacerClickCelda,
  alIntentarCelda,
  alArrastrarSobreCelda,
  seccionArrastrando,
  bloqueado = false,
  puedeInteractuarCelda,
}: MatrizCargaNoLectivaProps) {
          ...
          {matriz.filas.map((fila, idx) => (
            <tr key={idx} className="group hover:bg-slate-50/30 transition-colors">
              ...
              {fila.celdas.map((celda, cIdx) => {
                const interactuable =
                  !bloqueado &&
                  (puedeInteractuarCelda?.(celda) ?? false);

                const puedeRecibirArrastre =
                  interactuable &&
                  Boolean(
                    alArrastrarSobreCelda &&
                      seccionArrastrando &&
                ...
                return (
                  <td
                    key={cIdx}
                    onClick={() => {
                      alIntentarCelda?.(celda);
                      if (!interactuable) return;
                      alHacerClickCelda(celda.diaSemana, celda.horaInicio);
                    }}
                    onDragEnter={(e: React.DragEvent<HTMLTableCellElement>) => {
                      if (!puedeRecibirArrastre) return;
                      e.preventDefault();
                      alArrastrarSobreCelda?.(celda);
                    }}
                    ...
                    className={cn(
                      ...
                      puedeRecibirArrastre
                        ? 'cursor-copy group/celda ring-2 ring-inset ring-indigo-300/30 bg-indigo-50/40'
                        : interactuable
                        ? 'cursor-pointer group/celda'
                        : 'cursor-not-allowed',
                    ...
                    <div className="min-h-[42px] flex flex-col items-center justify-center p-1 text-center">
                      {celda.estado === 'LECTIVO' && (
                        <div className="flex flex-col gap-0.5">
                          ...
                          <span className="text-[8px] font-medium leading-none opacity-70 line-clamp-2">
                            {celda.info?.origen}
                          </span>
                        </div>
                      )}
                      ...
                      {celda.estado === 'NO_LECTIVO' && (
                        <div className="flex flex-col gap-0.5 animate-in zoom-in-95 duration-200">
                          ...
                          <span className="text-[8px] font-bold text-indigo-900 leading-none line-clamp-2">
                            {celda.info?.seccion
                              ? getLabelSeccion(celda.info.seccion)
                              : ''}
                          </span>
                        </div>
                      )}
                      ...
                      {celda.estado === 'LIBRE' && interactuable && (
                        <span className="text-indigo-300 font-bold text-lg opacity-0 group-hover/celda:opacity-100 transition-opacity">
                          +
                        </span>
                      )}
                    </div>
